...
`n/e/d/c` 会根据当前焦点新建、编辑、删除、复制 provider 或 model。方向键切换栏目和选择项目，`Enter` 进入模型列表，`Esc` 返回 provider，`i` 在线导入模型，Settings 的 Actions 中选择 `Import from OpenCode` 并按 `Enter` 从 `~/.config/opencode/opencode.json` 导入 provider 和 model，`Space` 设置默认模型，`b` 恢复备份，`v` 检查配置，`?` 查看全部键位。
